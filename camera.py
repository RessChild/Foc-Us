    ...
    def _thread(self):
        ...
        # 탐색해서 대상이 있다고 예상되는 사각형을 갖고 옴
                ...
                if len(rects) > 0: # 탐색한 놈이 1개 이상 있다면
                    ...
                    #    cv2.rectangle(self.frame, (x, y), (x+w, y+h), (0, 0, 255), 2) # 사각형을 그림
                    ...
                    if x1 + (w1//2) > 215: # 피사체 중심이 오른쪽으로 쏠린 경우
                        print("오른쪽 이동")
                        succ = True if servoControl(-1) else False # 그 방향으로 이동
                        if not succ:
                            print('오른쪽으로 최대치만큼 이동했습니다') # 초점을 맞추기 위해 더이상 움직일 수 없다면, 종료
                    elif x1 + (w1//2) < 185: #
                        print("왼쪽 이동")
                        succ = True if servoControl(1) else False
                        if not succ:
                            print('왼쪽으로 최대치만큼 이동했습니다') # 마찬가지
